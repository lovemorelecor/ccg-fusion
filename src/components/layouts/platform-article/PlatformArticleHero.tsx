...
export type PlatformArticleHeroProps = {
  ...
  titleId?: string
  badge?: string
  /** Use `p` when the page H1 lives in the article body (section-shared heroes). */
  titleElement?: 'h1' | 'p'
  imageMode?: 'visual' | 'background'
}
...
export function PlatformArticleHero({
  ...
  return (
    <section
      id="overview"
      className={`init-hero init-hero--with-section-nav pa-hero${imageMode === 'background' ? ' pa-hero--background' : ''}`}
      ...
      <div className="init-hero__inner pa-hero__inner">
        <div className="init-hero__text pa-hero__text">
          <div className="pa-hero__title-row">
            <TitleTag id={titleId} className="init-hero__heading pa-hero__title">
              {title}
            </TitleTag>
